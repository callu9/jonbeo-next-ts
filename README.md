...
- [x] Framer Motion을 사용한 인터렉션 구현
- [x] BFF 구현을 통한 mock API 개발
- [x] MTS 서비스 차트 구현
...
> (GPT 답변) 만약 다른 리소스 병목이 없고, LCP가 원래 2~3초 수준에서 측정된다고 가정하면:
...
2. TBT 개선폭:
   ...
   - 하지만 병목이 제거된 상황(예: 기본이 400ms였던 페이지라면 → 160ms 수준 감소)이라면 메인스레드 응답성이 크게 좋아져
     사용자가 터치/스크롤 시 바로 반응하는 경험을 할 수 있습니다.
```

물론 현재 Performance 점수가 워낙 낮아서 유의미하게 적용되진 않았지만, 병목이 제거된 상황이라면 LCP, TBT 측정 시간 단축이 유의미하게 작용될 것이라 추측하였습니다.<br />
앞으로의 과제는 Performance 측정에 영향을 주는 리소스 병목 제거가 되겠습니다!
